...
class QAOARequest(BaseModel):
    """
    Single endpoint request - everything in one call.
    ...
    expression: str = Field(
        description="Python expression using x[0], x[1], etc."
    )
    p: int = Field(
        default=1,
        ge=1,
        le=5,
        description="QAOA layers"
    )
    maxiter: int = Field(
        default=50,
        ge=10,
    ...
    backend: Literal["simulator", "ibm"] = Field(
        default="simulator",
        description="simulator or ibm"
    )
    shots: int = Field(
        default=1024,
        ge=100,
    ...
    device: Optional[str] = Field(
        default=None,
        description="IBM device name (optional)"
    )


class QAOAResponse(BaseModel):
    """
    Complete QAOA results.
    """
    success: bool
    optimal_cost: Optional[float] = None
    optimal_params: Optional[list] = None
    probabilities: Optional[Dict[str, float]] = None  # Normalized counts
    solution_counts: Optional[Dict[str, int]] = None  # Raw counts
